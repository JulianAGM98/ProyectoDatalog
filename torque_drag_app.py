...
def add_torque_drag_app():
    ...
    if st.checkbox('Set parameters:'):
        friction_factor = st.number_input('Friction Factor:', value=0.24, step=0.1)
    ...
    if trajectory is not None:
        ...
        dimensions = {'pipe': {'od': od_pipe, 'id': id_pipe, 'length': length_pipe, 'shoe': length_pipe},
                      'odAnn': od_annular}

        densities = {'rhof': rhof, 'rhod': rhod}

        result = td.calc(trajectory.trajectory, dimensions, densities, case='all', torque_calc=True, wob=wob, tbit=tbit,
